...
export const toggleAmount = (payload) => (dispatch) => {
  ...
  dispatch(countCartTotal())
  dispatch(updateShippingFee()) //calculate shipping fee
}
  ...
  reducers: {
    addToCart: (state, action) => {
      ...
      if (tempItem) {
        ...
        // TODO CROSS CHECK and write test ID productId + size !!!!!!!
        // TODO WRITE TEST
        const newItem = {
          id: productId + size,
          productId,
    ...
    updateShippingFee: (state, action) => {
      const isTotalAmountAbove30 = state.total_amount > 30
      const newShippingFee = isTotalAmountAbove30 ? 0 : (action.payload || 1.5)
      const changePresentStatus = isTotalAmountAbove30
      return {
        ...state,
        shipping_fee: newShippingFee,
        present: changePresentStatus,
      }
    },
